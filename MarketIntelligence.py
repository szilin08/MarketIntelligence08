...
# Mukim filter (filtered by selected districts)
if districts_sel:
    mukim_options = sorted(df[df["District"].isin(districts_sel)]["Mukim"].unique())
else:
    mukim_options = sorted(df["Mukim"].unique())
mukims_sel = st.sidebar.multiselect("Mukim", options=mukim_options, default=[])

# Scheme Name/Area filter (filtered by selected mukims or districts)
if mukims_sel:
    scheme_options = sorted(df[df["Mukim"].isin(mukims_sel)]["Scheme Name/Area"].unique())
elif districts_sel:
    scheme_options = sorted(df[df["District"].isin(districts_sel)]["Scheme Name/Area"].unique())
else:
...
if scheme_sel:
    filtered = filtered[filtered["Scheme Name/Area"].isin(scheme_sel)]
...
# -------------------------
    ...
    if df_sub.empty:
        ...
        mukim_year_agg = df_sub.groupby(["Mukim", "Year"])["Transaction Price"].mean().reset_index().rename(columns={"Transaction Price": "AvgPrice"})
        mukim_year_agg["Mukim"] = mukim_year_agg["Mukim"].astype(str).apply(standardize_name)
        mukim_year_agg = mukim_year_agg[mukim_year_agg["Year"] > 0]

        mukims = mukim_year_agg["Mukim"].dropna().unique().tolist()[:200]
        coords = geocode_places(mukims)
        mukim_year_agg["lat"] = mukim_year_agg["Mukim"].map(lambda x: coords.get(x, (None, None))[0])
        ...
        if points.empty:
            ...
            fig = px.scatter_mapbox(
                ...
                zoom=9,
                title=f"Avg Transaction Price by Mukim and Year in {parent}"
            )
            clicked = plotly_events(fig, click_event=True, hover_event=False, select_event=False)
            st.plotly_chart(fig, use_container_width=True)
